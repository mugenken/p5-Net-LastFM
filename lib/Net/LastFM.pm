...
This makes an unsigned request and returns the raw json:

  my $json = $lastfm->request( method => 'auth.gettoken' );

=head2 request_signed_json

This makes a signed request and returns the raw json:

  my $json = $lastfm->request_signed(
      method => 'user.getRecentTracks',
      user   => 'lglb',
  );
